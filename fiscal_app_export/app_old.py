...
def _validar_csv(filepath: str, exchange: str) -> tuple[bool, str]:
    ...
    if exchange == "binance":
        ...
        if not any(sig in primeras for sig in BITVAVO_SIGNATURES):
            return False, (
                "El fichero no parece ser un CSV de Bitvavo. "
                "Asegúrate de exportar el historial completo desde tu cuenta de Bitvavo."
            )
...
def procesar_bitvavo(filepath: str) -> MotorFIFO:
    """CSV Bitvavo → clasificador → motor FIFO."""
    c = ClasificadorBitvavo(filepath).clasificar()
    motor = MotorFIFO()
...
def analizar():
    ...
    try:
        ...
        if exchange == "bit2me":
            ...
            motor = procesar_bitvavo(tmp_path)
            ...
            pdf_bytes = generar_pdf(motor, nombre, ejercicio, "Bitvavo")
        else:
