...
def procesar_binance(filepath: str) -> tuple:
    ...
    motor = _pipeline_motor(c)
    return motor, c.rendimientos if hasattr(c, 'rendimientos') else []


def procesar_bitvavo(filepath: str) -> tuple:
    c = ClasificadorBitvavo(filepath).clasificar()
    motor = _pipeline_motor(c)
    return motor, c.rendimientos
...
def _motor_a_json(motor: MotorFIFO) -> tuple:
...
def _rendimientos_a_json(rendimientos: list) -> list:
    ...
    por_tipo = defaultdict(lambda: {"cantidad": 0.0, "operaciones": 0})
    ...
    return [
        {"subtipo": k, "activo": v["activo"], "cantidad": round(v["cantidad"], 6), "operaciones": v["operaciones"]}
